...
// TestIntegration_ArchB_Int32Guard 入账越界预检:写后将超 int32 → 拒,且出账已扣部分留 pending 给对账环。
func TestIntegration_ArchB_Int32Guard(t *testing.T) {
	...
	// 划 2 亿:成员写后 23 亿 > 21.47 亿 → Increase 预检拒(出账已扣,账本留 pending,对账环可见)。
	err := svc.Transfer(ctx, orgID, treasury, member.NewapiUserID, 0, 200_000_000, "topup", "i32-1", "test")
	if err == nil {
		t.Fatal("🔴写后超 int32 应拒")
	}
	drifts, rerr := svc.ReconcileTransfers(ctx)
	if rerr != nil {
		t.Fatalf("对账环失败: %v", rerr)
	}
	// pending 行 2 分钟阈值内不会被抓取——直接查账本确认 pending 留存(对账环输入)。
	var status string
	if qerr := store.DB().QueryRowContext(ctx, `SELECT status FROM ledger_transfer WHERE idempotency_key='i32-1'`).Scan(&status); qerr != nil {
		t.Fatalf("查账本失败: %v", qerr)
	}
	if status != "pending" {
		t.Fatalf("🔴中途失败应留 pending(对账环收敛),得 %s", status)
	}
	_ = drifts
	t.Log("ArchB int32 guard ok: 越界拒 + 中途失败留 pending")
}
